...
class HastaAnaEkrani(QWidget):
    ...
    def init_ui(self):
        ...
        # Always show historical chart (all records)
        self._show_gecmis_grafik()
    ...
    def _show_gecmis_grafik(self):
        """Fetch ALL blood sugar records and draw a historical trend chart."""
        try:
            self.cursor.execute("""
            ...
            """, (self.hasta_id,))
            tum_olcumler = self.cursor.fetchall()
            ...
            zamanlar = []
            degerler = []
            for ts, seviye in tum_olcumler:
                try:
                    dt = ts if hasattr(ts, 'year') else _dt.fromisoformat(str(ts))
                    zamanlar.append(dt)
                    degerler.append(seviye)
                except Exception:
                    pass
            ...
            ax.plot(zamanlar, degerler, linewidth=2.5, color='#3b82f6',
                    marker='o', markersize=6, markerfacecolor='white',
                    markeredgecolor='#3b82f6', markeredgewidth=2, label='Kan Şekeri')

            ax.axhspan(70, 110, alpha=0.15, color='#16a34a', label='Normal (70–110)')
            ax.axhspan(110, 180, alpha=0.15, color='#facc15', label='Hafif Yüksek')
            ax.axhspan(180, max(degerler + [181]), alpha=0.15, color='#f87171', label='Hiperglisemi')
            ax.axhspan(0, 70, alpha=0.15, color='#60a5fa', label='Hipoglisemi')

            ax.set_title("Tum Zamanlar - Kan Sekeri Gecmisi", fontsize=14,
                         color='#1e40af', fontweight='bold', pad=14)
            ax.set_xlabel("Tarih / Saat", fontsize=11, color='#374151')
            ax.set_ylabel("mg/dL", fontsize=11, color='#374151')
            ...
            gecmis_card = self.create_info_card("📊 Geçmiş Kan Şekeri Grafiği", canvas)
            self.layout.addWidget(gecmis_card)

        except Exception as e:
            print("Geçmiş grafik hatası:", e)
    ...
    def show_modern_olcum_tablosu(self, olcumler):
        """Show a table of ALL historical blood sugar records."""
        # Fetch all records, not just today's
        self.cursor.execute("""
            SELECT tarih_zaman, kan_sekeri
            FROM kan_sekeri
            WHERE hasta_id = ?
        ...
        tablo.setHorizontalHeaderLabels(["Tarih", "Saat", "Kan Sekeri (mg/dL)"])
        ...
        for i, (ts, seviye) in enumerate(tum_olcumler):
            try:
                ...
                dt = None
            tarih = dt.strftime("%d.%m.%Y") if dt else str(ts)
            saat = dt.strftime("%H:%M") if dt else ""

            tarih_item = QTableWidgetItem(tarih)
            saat_item = QTableWidgetItem(saat)
            seviye_item = QTableWidgetItem(str(seviye))

            dark_text = QColor("#111827")
            tarih_item.setForeground(dark_text)
            saat_item.setForeground(dark_text)
            seviye_item.setForeground(dark_text)
        ...
        tablo_card = self.create_info_card("Tum Olcum Gecmisi", tablo)
    ...
    def show_durum_kartlari(self):
        ...
        self.cursor.execute("""
            ...
            WHERE e.hasta_id = ? AND date(e.tarih_zaman) = date('now')
        """, (self.hasta_id,))
    ...
    def show_modern_uyarilar(self):
        self.cursor.execute("""
            ...
            WHERE hasta_id = ? AND date(zaman) = date('now')
        """, (self.hasta_id,))
    ...
    def sifre_degistir(self):
        ...
        if ok and yeni_sifre:
            ...
            try:
                self.cursor.execute("UPDATE hastalar SET sifre = ? WHERE id = ?", (hashli, self.hasta_id))
                self.conn.commit()
